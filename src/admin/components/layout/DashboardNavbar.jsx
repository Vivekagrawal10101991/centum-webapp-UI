    ...
    try {
      ...
      if (osmResponse.ok) {
        ...
        if (osmData && osmData.address) {
          const { building, amenity, road, residential, neighbourhood, suburb, village, town, city_district, city, state } = osmData.address;
          ...
          if (uniqueOsmParts.length > 0) return uniqueOsmParts.join(', ');
        }
    ...
    try {
      ...
      if (photonRes.ok) {
        ...
        if (photonData.features && photonData.features.length > 0) {
          ...
          if (uniquePhotonParts.length > 0) return uniquePhotonParts.join(', ');
        }
      }
    } catch (photonError) {
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to check out? Once checked out, you will not be able to check back in for the rest of the day."
